...
func (r *MySQLSessionRepository) GetCurrentSession(roomID string) (*model.AuctionSession, error) {
	row := r.db.QueryRow(`
		SELECT s.id, s.room_id, s.item_id, s.status, s.current_price, s.leader_user_id, s.end_time,
		       s.participant_count, s.increment_step, s.extension_seconds, s.extension_trigger_seconds,
...
func (r *MySQLSessionRepository) ListRoomSessions(roomID string) ([]model.AuctionSession, error) {
	rows, err := r.db.Query(`
		SELECT id, room_id, item_id, status, current_price, leader_user_id, end_time,
...
func (r *MySQLSessionRepository) SaveSession(session model.AuctionSession) error {
	_, err := r.db.Exec(`
		INSERT INTO auction_sessions (id, room_id, item_id, status, current_price, leader_user_id, end_time, participant_count, increment_step, extension_seconds, extension_trigger_seconds, ceiling_price, supports_auto_proxy)
		VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
